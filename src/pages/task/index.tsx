    ...
    <div className="jakarta grid grid-cols-1 gap-8 p-7">
      ...
      <div className="grid grid-cols-1 grid-flow-col items-center">
        ...
        <div className="flex justify-end gap-2">
          <MdChevronLeft size="24px" cursor="pointer" onClick={handleTaskPrevClick} />
          <MdChevronRight size="24px" cursor="pointer" onClick={handleTaskNextClick} />
        </div>
      </div>
      <div className="grid place-items-center grid-cols-1 md:grid-cols-4 lg:grid-cols-4 sm:grid-cols-2 gap-8">
        ...
        <div className="hidden md:block">
          {taskData[(taskIndex + 1) % taskData.length] && (
            <TaskCard key={(taskIndex + 1) % taskData.length} {...taskData[(taskIndex + 1) % taskData.length]} />
          )}
        </div>
        <div className="hidden md:block">
          {taskData[(taskIndex + 2) % taskData.length] && (
            <TaskCard key={(taskIndex + 2) % taskData.length} {...taskData[(taskIndex + 2) % taskData.length]} />
          )}
        </div>
        <div className="hidden md:block">
          {taskData[(taskIndex + 3) % taskData.length] && (
            <TaskCard key={(taskIndex + 3) % taskData.length} {...taskData[(taskIndex + 3) % taskData.length]} />
          )}
        </div>
      </div>
      <div className="grid grid-cols-1 grid-flow-col items-center">
